...
def create_visualization_plots(swap_results_tuple, zero_random_results, latents_pca, latents_tsne, properties, explained_variance=None, run_dir=None):
    ...
    if correct_recons and swap_results:
        ...
        print(f"  [OK] Latent swap plot saved: {fname}")
        plt.close()
    else:
        print("  [WARNING] No swap results, skipping latent swap plot.")
    ...
    if zero_random_results['zero'] and zero_random_results['correct']:
        ...
        print(f"  [OK] Zero/random latent plot saved: {fname}")
        plt.close()
    else:
        print("  [WARNING] No zero/random results, skipping zero/random latent plot.")
    ...
    if latents_pca is not None and len(latents_pca) > 0:
        ...
        print(f"  [OK] Latent space plot saved: {fname}")
        plt.close()
    else:
        print("  [WARNING] No latent PCA data, skipping latent space plot.")
...
def run_latent_validation_for_specialist(run_dir, model, device, eval_keys, n_samples_per_key=5, wandb_logger=None, step_hint=None):
    ...
    for key in eval_keys[:2]:
        try:
            ...
            print(f"  [OK] Generated {len(inputs)} samples for key '{key}'")
        except Exception as e:
    ...
    try:
        ...
        if latent_plots:
            print(f"  [ OK ] Generated plots: {list(latent_plots.keys())}")
            for k, v in latent_plots.items():
                print(f"    - {k}: {v}")
        else:
            print("  [ WARNING ] No plots were generated by latent validation.")
        ...
        if wandb_logger:
            ...
            for plot_name, plot_path in latent_plots.items():
                if os.path.exists(plot_path):
                    ...
                    print(f"  [ OK ] Uploaded {plot_name} to WandB: {plot_path}")
                else:
                    print(f"  [ WARNING ] Plot file missing for WandB upload: {plot_path}")
            wandb_logger._safe_log(log_dict, step_hint=step_hint)
        print("  [ OK ] Latent validation complete.")
        return {
            'success': True,
...
def main():
    ...
    if wandb_settings.get('enabled', False):
        ...
        if wandb_logger:
            print(f"[ OK ] WandB logging enabled: {wandb_logger.run.name}")
    ...
    if wandb_logger:
        ...
        try:
            ...
            print(f"[ OK ] Results logged to WandB at epoch {args.epoch}")
