...
# DEPRECATED FUNCTIONS REMOVED - Use plot_comprehensive_latent_space instead
...
def plot_multi_encoder_evaluation_results(key, key_results, save_dir=None):
    ...
    try:
        ...
        try:
            ...
            print(f"❌ Fallback evaluation also failed: {str(fallback_error)}")
            # Create a simple error plot
            plt.figure(figsize=(8, 6))
            plt.text(0.5, 0.5, f'Evaluation visualization failed for {key}\n\nOriginal error: {str(e)}\nFallback error: {str(fallback_error)}', 
                     ha='center', va='center', transform=plt.gca().transAxes, fontsize=12)
            plt.title(f'Evaluation Error: {key}')
            if save_dir:
                error_filename = f'evaluation_error_{key}.png'
                plt.savefig(os.path.join(save_dir, error_filename))
                plt.close()
            else:
                plt.show()
...
def plot_enhanced_model_summary(results, model_params, save_dir=None):
    ...
    try:
        ...
        print(f"Error in enhanced model summary: {e}")
        return
    ...
    try:
        ...
        # Process evaluation data (unified format for both single and multi-encoder)
            if eval_results:
                ...
                # FALLBACK: If no training latent data in evaluation, try to collect from training results
                if 'training_latent_data' not in key_results:
                    print("    No training latent data in evaluation results, attempting fallback collection...")
                    # Try to find run directory from save_dir and collect training data
                    if save_dir and os.path.exists(os.path.join(save_dir, 'results.pkl')):
                        try:
                            with open(os.path.join(save_dir, 'results.pkl'), 'rb') as f:
                                training_results = pickle.load(f)
                            
                            # Check if we have latent data in training results
                            if 'latent_zs' in training_results and len(training_results['latent_zs']) > 0:
